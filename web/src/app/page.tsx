...
function buildWaUrl(recipientPhone: string, senderName: string, verifyUrl: string, lang: Lang): string {
  ...
  return `https://wa.me/${normalizePhone(recipientPhone)}?text=${encodeURIComponent(msg)}`;
}
...
function buildSmsUrl(recipientPhone: string, senderName: string, verifyUrl: string, lang: Lang): string {
  const msg = lang === 'he'
    ? `${senderName} מבקש לאמת את זהותך. לחץ כאן: ${verifyUrl}`
    : `${senderName} is asking you to verify your identity. Tap here: ${verifyUrl}`;
  return `sms:${normalizePhone(recipientPhone)}?body=${encodeURIComponent(msg)}`;
}
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    if (p.name && p.phone) {
      ...
      // Pre-fill country code for onboarding
  ...
  // Update recipient prefix & default message when lang changes
  ...
  useEffect(() => {
    ...
    // Only update recipient if it still looks like a bare country code
  ...
  function handleSaveEdit() {
    ...
    savePrefs({ name: draftName.trim(), phone: draftPhone });
  ...
  async function handleSend(forcePlatform?: Platform) {
    ...
    try {
      ...
      const url = usePlatform === 'whatsapp'
        ? buildWaUrl(recipientPhone, myName, data.verification_url, lang)
        : buildSmsUrl(recipientPhone, myName, data.verification_url, lang);
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '0.78rem', color: '#6b7280', direction: 'ltr' }}>{myPhone}</div>
              </div>
            </div>
            ...
            <>
              ...
              <button onClick={() => {
                const url = platform === 'whatsapp'
                  ? buildSmsUrl(recipientPhone, myName, verifyUrl, lang)
                  : buildWaUrl(recipientPhone, myName, verifyUrl, lang);
                window.open(url, '_blank');
